...
class GuideController extends CController
{
	...
	{
		$topic=$this->getTopic();
		$file=Yii::getPathOfAlias("webroot.guide.{$this->language}").DIRECTORY_SEPARATOR.$topic.'.txt';
		if(!is_file($file))
			$file=Yii::getPathOfAlias("webroot.guide.source").DIRECTORY_SEPARATOR.$topic.'.txt';
		...
		$imageUrl=Yii::app()->baseUrl.'/guide/source/images';
	...
	public function getTopics()
	{
		...
		{
			$file=Yii::getPathOfAlias("webroot.guide.{$this->language}.toc").'.txt';
			if(!is_file($file))
				$file=Yii::getPathOfAlias('webroot.guide.source.toc').'.txt';
			$lines=file($file);
			$chapter='';
	...
	public function getLanguages()
	{
		...
		{
			$basePath=Yii::getPathOfAlias('webroot.guide');
