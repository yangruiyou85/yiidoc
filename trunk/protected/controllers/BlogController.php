...
class BlogController extends CController
{
	...
	{
		$topic=$this->getTopic();
		$file=Yii::getPathOfAlias("webroot.blog.{$this->language}").DIRECTORY_SEPARATOR.$topic.'.txt';
		if(!is_file($file))
			$file=Yii::getPathOfAlias("webroot.blog.source").DIRECTORY_SEPARATOR.$topic.'.txt';
		...
		$imageUrl=Yii::app()->baseUrl.'/blog/source/images';
	...
	public function getTopics()
	{
		...
		{
			$file=Yii::getPathOfAlias("webroot.blog.{$this->language}.toc").'.txt';
			if(!is_file($file))
				$file=Yii::getPathOfAlias('webroot.blog.source.toc').'.txt';
			$lines=file($file);
			$chapter='';
	...
	public function getLanguages()
	{
		...
		{
			$basePath=Yii::getPathOfAlias('webroot.blog');
